...
if($row['cnt']>0) {
	...
	if(!$row2['cnt']) {
		$sql = "insert into {$g5['eyoom_theme']} set tm_name='{$tm_theme}', tm_alias='{$tm_alias}', tm_host='{$hostname}', tm_time='".time()."'";
	} else {
		$sql = "update {$g5['eyoom_theme']} set tm_alias='{$tm_alias}' where tm_name='{$tm_theme}'";
	}
	sql_query($sql,false);
}
?>
