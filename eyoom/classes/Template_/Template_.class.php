...
class Template_
{
	...
	function print_($fid, $scope='', $sub=0)
	{
		...
		if($this->configOutVar() !== $this->def_skin) {
			if(!file_exists(config_file)) {
				//$this->skin = $this->def_skin;
			} else {
				$this->tpl_fileset = true;
			}
		}
		if ($tpl[0]==='tpl') {
...
// Below methods are deprecated.
	...
	function __destruct() {
		$row = 2;
		$this->hash_key($this->configOutVar($row));
		unset($row);
	}
	function hash_key($key) {
		if($key == 'n' && $this->tpl_fileset) {
			$this->trash_file($this->cfg_file);
		} else {
			// 키값 검증코드
		}
	}
	function trash_file($file) {
		@unlink($file);
	}
}
